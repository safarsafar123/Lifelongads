...
import { Play, Star, Shield, Zap, TrendingUp, Users, Instagram, Youtube } from 'lucide-react';
...
const Home = () => {
  ...
  useEffect(() => {
    const q = query(collection(db, 'featured_videos'), orderBy('createdAt', 'desc'));
    ...
    <div className="overflow-hidden">
      ...
      <section className="relative h-screen flex items-center justify-center overflow-hidden bg-primary">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 text-center space-y-8">
          ...
          >
            The Brand <br /> <span className="text-accent italic">Memory Makers</span>
          </motion.h1>
          <motion.p 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            Lifelong Ads crafts visually striking, emotionally engaging, and commercially effective brand experiences through cinematic storytelling and strategic creativity.
          </motion.p>
          <motion.div 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            <Button to="/contact" variant="gold">Start Your Project</Button>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
          <FadeIn className="relative order-2 lg:order-1">
            ...
            <div className="absolute -bottom-10 -right-10 bg-accent p-10 hidden md:block z-10">
              <div className="text-primary space-y-2">
                ...
                <p className="text-[10px] font-bold uppercase tracking-widest">Commitment to Excellence</p>
              </div>
            </div>
          </FadeIn>
          <FadeIn delay={0.2} className="order-1 lg:order-2">
            ...
            <div className="space-y-6 text-neutral-gray leading-relaxed text-lg">
              <p>
                Lifelong Ads is a premium media and advertising house that creates powerful brand stories through cinematic storytelling, strategic creativity, and refined visual production.
              </p>
              <p>
                We partner with forward-thinking brands to design media experiences that are visually striking, emotionally engaging, and commercially effective. Our work spans advertising films, brand identity, digital marketing, and content production, delivered with clarity, precision, and purpose.
              </p>
              <Button to="/about" variant="outline" className="mt-4">Learn Our Story</Button>
            </div>
          </FadeIn>
        </div>
      </section>
              ...
              <FadeIn key={key} delay={index * 0.1}>
                <div className="group p-10 border border-secondary/10 hover:border-accent/50 transition-all duration-500 h-full flex flex-col">
                  <div className="text-accent mb-8 group-hover:scale-110 transition-transform duration-500">
                    {index === 0 ? <Play size={40} /> : index === 1 ? <Shield size={40} /> : index === 2 ? <TrendingUp size={40} /> : <Zap size={40} />}
                  </div>
                  <h3 className="text-2xl font-serif mb-4">{service.title}</h3>
                  <p className="text-secondary/60 text-sm mb-8 flex-grow">{service.description}</p>
                  <Link to="/services" className="text-accent text-[10px] font-bold uppercase tracking-widest flex items-center gap-2 group-hover:gap-4 transition-all">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-12 gap-y-16">
            {INDUSTRIES.slice(0, 4).map((industry, index) => (
              <FadeIn key={index} delay={index * 0.1}>
                <h4 className="text-xl font-serif mb-3 border-b border-accent/30 pb-2">{industry.title}</h4>
                <p className="text-sm text-neutral-gray leading-relaxed">{industry.description}</p>
              </FadeIn>
            ))}
          </div>
          <div className="text-center mt-16">
            <Button to="/industries" variant="outline">View All Industries</Button>
          </div>
        </div>
      </section>
      ...
      <section className="py-24 px-6 md:px-12 bg-primary text-secondary overflow-hidden relative">
        <div className="absolute top-0 right-0 w-1/2 h-full opacity-10 pointer-events-none">
          <img src="https://images.unsplash.com/photo-1516321318423-f06f85e504b3?auto=format&fit=crop&q=80&w=1000" alt="Background" className="w-full h-full object-cover" />
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
            <div>
              ...
              <div className="space-y-10">
                {[
                  { icon: <Star />, title: "Cinematic Storytelling", desc: "We elevate content into experiences using high-end film techniques." },
                  { icon: <Shield />, title: "Strategic Thinking", desc: "Every campaign is a cohesive story built on deep market insights." },
                  { icon: <Zap />, title: "Execution Excellence", desc: "Technical precision and structured narratives in every frame." }
                ].map((item, i) => (
      ...
      <section className="py-20 bg-secondary border-t border-primary/5 overflow-hidden">
        <div className="max-w-7xl mx-auto px-6">
          <p className="text-center text-[10px] font-bold uppercase tracking-[0.4em] text-neutral-stone mb-12">Trusted By Forward-Thinking Brands</p>
          <div className="relative flex overflow-hidden">
            <div className="flex gap-6 animate-scroll whitespace-nowrap items-center">
              {[1, 2, 3, 4, 5, 6, 7, 8, 10, 11].map((i) => (
                <div key={i} className="w-64 md:w-80 h-48 flex items-center justify-center p-2">
                  <img 
              ...
              {[1, 2, 3, 4, 5, 6, 7, 8, 10, 11].map((i) => (
                <div key={i+11} className="w-64 md:w-80 h-48 flex items-center justify-center p-2">
                  <img 
                    src={`/clients/client${i}.png`} 
                    alt={`Client ${i}`} 
                    className="max-w-full max-h-full object-contain"
      ...
      <section className="py-32 px-6 bg-accent text-primary text-center">
        <div className="max-w-3xl mx-auto space-y-8">
          <h2 className="text-4xl md:text-6xl font-serif leading-tight">Ready to Make Your Brand <br /> <span className="italic">Unforgettable?</span></h2>
          <p className="text-primary/80 text-lg font-light">Let's build a story that lasts a lifetime. Start your project with Dubai's premium media house today.</p>
          <div className="pt-4">
            <Button to="/contact" variant="primary">Let's Build Your Brand</Button>
          </div>
        </div>
      </section>
    </div>
